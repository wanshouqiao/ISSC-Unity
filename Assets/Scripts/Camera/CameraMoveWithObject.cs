...
public class AdvancedCameraFocus : MonoBehaviour
{
    ...
    [Header("静止点击设置")]
    [Tooltip("判定静止的最大像素移动量")]
    [SerializeField] float _moveThreshold = 5f;
    [Tooltip("点击有效时间窗口(秒)")]
    [SerializeField] float _clickWindow = 0.2f;
    ...
    void Update()
    {
        ...
        {
            StartCoroutine(ClickStabilityCheck());
        }
        ...
        if (Input.GetMouseButtonUp(0))
        {
            if (_isValidClick)
            {
                if (Physics.Raycast(_mainCam.ScreenPointToRay(Input.mousePosition), out RaycastHit hit))
                {
    ...
    IEnumerator ClickStabilityCheck()
    {
        _mouseDownPos = Input.mousePosition;
        float timer = 0;

        while (timer < _clickWindow)
        {
            // 实时检测鼠标移动量
            if (Vector2.Distance(_mouseDownPos, Input.mousePosition) > _moveThreshold)
            {
                yield break;
            }
